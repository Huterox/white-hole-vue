        ...
        <div style="height:100px">
          ...
          <div style="display:inline-block;margin-left: 5%;width: 60%">
            <p class="message" style="font-weight:bold" >
              <a style="cursor:pointer;color: #ea5050" class="alink" @click="goto(message)"  >
                {{message.quizTitle}}
              </a>
            </p>
            <p style="font-weight: lighter;color: #0bc4e5" class="message"
...
export default {
  ...
  methods:{
    ...
    getDataList(){
      ...
      let pageSession = sessionStorage.getItem("hotQuizPageSession");
      let total = sessionStorage.getItem("hotQuizTotal");
      if(pageSession && total){
        ...
        this.axios({
          ...
          if (res.code === 0) {
            ...
            //存储临时缓存
            sessionStorage.setItem("hotQuizPageSession", JSON.stringify(this.Messages));
            sessionStorage.setItem("hotQuizTotal",page.totalCount);
          } else {
